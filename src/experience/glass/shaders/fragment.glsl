...
void main () {
    ...
    for (int i = 0; i < LOOP; i++) {
        ...
        vec3 refractVecY = refract(eyeVector, normal, (1.0/(uIorY + 1.03 * uOffset)));
        vec3 refractVecG = refract(eyeVector, normal, (1.0/(uIorG + 1.04 * uOffset)));
        vec3 refractVecC = refract(eyeVector, normal, (1.0/(uIorC - 1.07 * uOffset)));
        vec3 refractVecB = refract(eyeVector, normal, (1.0/(uIorB - 1.05 * uOffset)));
        vec3 refractVecP = refract(eyeVector, normal, (1.0/(uIorP + 1.05 * uOffset)));
